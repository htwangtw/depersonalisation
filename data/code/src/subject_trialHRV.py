...
for subject in ID_list[0:1]:
    print(subject)
    event_path = p / "data" / subject / "func" / f"{subject}_task-heartbeat_run-1_events.tsv"
    physio_path = p / "data" / subject / "func" / f"{subject}_task-heartbeat_run-1_physio.tsv.gz"
    ...
    # interpolate nan
    f = interpolate.interp1d(ibi_timestamp[keep_idx], 
                             full_ibi[keep_idx], 
    ...
    for f in ['lf', 'hf']:
        ...
        amptitude = np.trapz(y=psd[idx_freq, :], 
                             dx=np.diff(freq)[0], axis=0)  
        plt.plot(t, amptitude)

    hrv_stats = pd.DataFrame(None, 
                           columns=["lf_power", "hf_power", 
                                      "rmssd", "n_peak", "bpm", "qc"],
    ...
    hrv_stats.to_csv(p / "scratch" / "trial_HRV" / f"{subject}_task-heartbeat_run-1_desc-HRV_events.tsv", sep="\t")
