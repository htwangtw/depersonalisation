...
def infotodict(seqinfo):
    ...
    task_interoception = create_key('sub-{subject}/func/sub-{subject}_task-heartbeat_run-{item:01d}_bold')
    task_rs = create_key('sub-{subject}/func/sub-{subject}_task-rest_echo-{item:01d}_bold')
    ...
    info = {t1w: [], task_rs:[], task_interoception:[], 
            noddi_300:[], noddi_800:[], noddi_2400:[]}
    ...
    for s in seqinfo:
        ...
        * image_type
        """
        if (s.dim1 == 256) and (s.dim2 == 240) and ('HiResMPRAGE_WIDER_GRAPPA' in s.protocol_name):
            info[t1w].append(s.series_id)
        elif (s.dim1 == 64) and (s.dim2 == 64) and ('multi-echo RS fMRI' in s.protocol_name):
            info[task_rs].append(s.series_id)
        elif (s.dim1 == 64) and (s.dim2 == 64) and ('ep2d_fid_INTEROCEPTION' in s.protocol_name):
