...
target_space = Path(
    home
    + "/projects/critchley_depersonalisation/data/derivatives/func_smooth-6mm/sub-10048/func/sub-10048_task-heartbeat_run-1_space-MNI152NLin2009cAsym_desc-preproc-fwhm6mm_bold.nii.gz"
)
for seed_number in [86, 87, 88, 89, 92, 93]:
    ...
    bin_nii = nb.Nifti1Image(
        (data > 0).astype(int), header=thresh.header, affine=thresh.affine
    )
    file_name = str(
        Path(home + "/projects/critchley_depersonalisation/references/ppi_seeds")
    )
